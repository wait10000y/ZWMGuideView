...
- (void)configureItemsFrame
{
    ...
    // 描述文字
    NSString *descStr = [self.dataSource guideMaskView:self descriptionLabelForItemAtIndex:self.currentIndex];
    self.textLabel.text = descStr?:@"";
    ...
    switch (itemRegion)
    {
        case ZWMGuideMaskItemRegionLeftTop:
        {
            ...
            textRect = [self.textLabel textRectForBounds:CGRectMake(0, 0, maxWidth, CGFLOAT_MAX) limitedToNumberOfLines:NSIntegerMax];
            // 文字位置左下方.
            CGFloat x = CGRectGetMidX(obtainFrame) - CGRectGetWidth(textRect) * 0.5;
            if (x<descInsetsX) {
                x = descInsetsX;
            }
            textRect.origin.x = x;
            textRect.origin.y = CGRectGetMaxY(arrowRect) + space;
            ...
            if (disHeight< 0) {
                if (-disHeight <= space*2) {
                    ...
                    if (CGRectGetMaxY(obtainFrame)+space +CGRectGetHeight(textRect) <= CGRectGetHeight(self.bounds)) {
        ...
        case ZWMGuideMaskItemRegionRightTop:
        {
            ...
            CGFloat x = CGRectGetMidX(obtainFrame)-textWidth*0.5;
            if (x+textWidth >CGRectGetWidth(self.bounds)-descInsetsX) {
                x = CGRectGetWidth(self.bounds)-descInsetsX-textWidth;
            }
            textRect.origin.x = x;
            textRect.origin.y = CGRectGetMaxY(arrowRect) + space;
        ...
        case ZWMGuideMaskItemRegionLeftBottom:
        {
            ...
            textRect.origin.x = x;
            textRect.origin.y = CGRectGetMinY(arrowRect) - space - textHeight;
            ...
            CGFloat disHeight = CGRectGetHeight(self.bounds) - CGRectGetMinY(textRect)-space - textHeight;
            if (disHeight< 0) {
                if (-disHeight <= space*2) {
                    arrowRect.origin.y -= disHeight/2;
                    textRect.origin.y -= disHeight;
                    ...
                    arrowRect = CGRectZero;
                    if (CGRectGetMinY(obtainFrame)-space -textHeight >= 0) {
...
//            CGFloat textWidth = CGRectGetWidth(textRect);
            ...
            textRect.origin.x = x;
            textRect.origin.y = CGRectGetMidY(obtainFrame)-textHeight*0.5;
            
            if (textHeight > CGRectGetHeight(self.bounds)-descInsetsX*2) {
                if (textHeight <= CGRectGetHeight(self.bounds)) {
                    textRect.origin.y = (CGRectGetHeight(self.bounds)-textHeight)/2;
                }else{
                    textRect.origin.y = 0;
                }
            }else{
        ...
        case ZWMGuideMaskItemRegionRight: {
            ...
            CGFloat textWidth = CGRectGetWidth(textRect);
            CGFloat textHeight = CGRectGetHeight(textRect);
            CGFloat x = CGRectGetMinX(arrowRect) - space - textWidth;
            textRect.origin.x = x;
            textRect.origin.y = CGRectGetMidY(obtainFrame)-textHeight*0.5;
